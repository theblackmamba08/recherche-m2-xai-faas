"""Génère code/notebooks/softcam-cluster4-v3-runB5.ipynb (Phase 2 — H1 v3 Run B5).

Run B5 : DERNIER TEST avant pivot. Tous les bons ingrédients + mix=0.05.
...
Historique :
  ...
  Run B4 (mix=0.10 const, γ=0, LayerNorm)          : R²=-3.58, Spear=0.44 (pire!)
  Run B5 (warm-up→0.05, anneal γ, LayerNorm)       : ?

Leçon Run B4 : sans warm-up, R² s'effondre même avec mix petit. Le warm-up est
l'ingrédient critique — il permet au décodeur d'apprendre proprement avant
qu'on injecte h_evidence.

Run B5 combine TOUS les ingrédients qui ont aidé jusqu'ici :
  - warm-up mix : 0 (epochs 0-14) → ramp (15-34) → 0.05 (35+)
  - anneal γ   : 0 (epochs 0-24) → ramp (25-39) → 1e-3 (40+)
  - LayerNorm sur h_evidence (v3)
  - mix cible = 0.05 (plus petit que B3=0.3 et B4=0.10)

Si Run B5 PASS → H1 défendable.
Si Run B5 FAIL → preuve définitive que l'architecture additive ne marche pas.
                  → pivot H2 (TsSHAP / SHAPformer).

Lance : python _generate_softcam_cluster4_v3_runB5.py
"""

import json
from pathlib import Path

NOTEBOOK_PATH = Path(__file__).parent / "softcam-cluster4-v3-runB5.ipynb"
...
# ──────────────────────────────────────────────────────────────────────────
md(
    """# SoftCAM-Transformer v3 — Run B5 (dernier test : tous les fixes + mix=0.05)

## Récapitulatif des 5 runs précédents
...
1. **Sans warm-up, R² s'effondre** : B4 (mix=0.10 sans warm-up) est pire que B3 (mix=0.3 avec warm-up). Le warm-up est l'ingrédient critique.
2. **L'anneal γ et le LayerNorm aident** : B2 → B3 montre une légère amélioration (+0.4 pp R², M passe de Dirac à diffuse).
3. **Mix petit aide *quand combiné avec warm-up*** : c'est ce que B5 teste.

## Configuration Run B5 — toutes les bonnes idées combinées

| Paramètre | Valeur |
|-----------|--------|
| `evidence_mix` cible | **0.05** (plus petit que tous les runs précédents) |
| Warm-up mix | epochs 0-14 (mix=0) → ramp → 0.05 (35+) |
| `gamma_entropy` cible | 1e-3 |
| Anneal γ | epochs 0-24 (γ=0) → ramp → 1e-3 (40+) |
| `beta_l2` | 1e-3 |
| LayerNorm | ✅ (v3) |

## Verdicts possibles

- **PASS H1.C** (R²≥0.30, Spearman≥0.85) → H1 défendable : *« mix=0.05 préserve la précision tout en fournissant M algébriquement exacte »*.
- **FAIL** → preuve empirique que l'architecture additive est fondamentalement incompatible avec encoder-decoder TimeSeriesTransformer. Pivot H2.
...
code(
    """# ── Hyperparams (identiques aux runs précédents) ───────────────
FREQ              = '1T'
...
# ── Evidence Layer v3 (Run B5 — tous les fixes + mix=0.05) ──
USE_EVIDENCE_LAYER   = True
EVIDENCE_MIX_TARGET  = 0.05          # ← très petit (vs 0.10 en B4, 0.3 en B3)
ALPHA_L1             = 0.0
BETA_L2              = 1e-3
GAMMA_ENTROPY_TARGET = 1e-3

# ── Schedules (identiques B2/B3 — warm-up est critique d'après B4) ──
MIX_WARMUP_END     = 15
MIX_RAMP_END       = 35
...
CLUSTER_ID = 4
RUN_NAME   = f'softcam-cluster{CLUSTER_ID}-v3-runB5'
print(f'Run : {RUN_NAME}')
print(f'Modèle : SoftCAMTransformerV3ForPrediction (Fix #4 LayerNorm)')
print(f'mix target = {EVIDENCE_MIX_TARGET}  (plus petit que tous les runs précédents)')
...
print()
print(f'C\\'est le dernier test avant pivot. Si FAIL : architecture additive prouvée incompatible.')"""
)
...
axes[0].axhline(0.1, color='red', linestyle=':', alpha=0.5, label='B4 (FAIL)')
axes[0].axhline(0.3, color='red', linestyle='--', alpha=0.5, label='B3 (FAIL)')
axes[0].set_xlabel('Epoch'); axes[0].set_ylabel('evidence_mix')
axes[0].set_title('Schedule mix (cible 0.05 — plus petit que tous les runs précédents)')
...
axes[1].set_title('Schedule γ (identique B2/B3)')
axes[1].grid(alpha=0.3)

plt.suptitle('Run B5 — tous les fixes + mix=0.05', fontsize=12)
plt.tight_layout()
plt.show()"""
)
...
md(
    """## 10 — Entraînement 51 epochs (warm-up + anneal, mix cible 0.05)
...
ckpt_path = f'{DRIVE_BASE}/checkpoints/v3_runB5_final.pt'
...
plt.suptitle(f'Run B5 — v3 + warm-up + anneal + mix={EVIDENCE_MIX_TARGET}', fontsize=13)
plt.tight_layout()
...
results = {
    'run': RUN_NAME,
    'mode': 'evidence-layer-on-warmup-anneal-layernorm-tiny-mix',
    'model_version': 'v3',
    'fix_combo': 'warm-up + anneal γ + LayerNorm + mix=0.05',
    'use_evidence_layer': USE_EVIDENCE_LAYER,
    ...
    'runb4_r2': RUNB4_R2, 'runb4_spear': RUNB4_SPEAR,
    ...
    'delta_r2_vs_runb4_pp': (test_r2  - RUNB4_R2) * 100,
...
print()
print('=' * 72)
print('  GATE H1.C — SoftCAM v3 Run B5 (DERNIER test, mix=0.05 + tous les fixes)')
print('=' * 72)
...
print(f'  Δ R²   vs Run B4 : {(test_r2-RUNB4_R2)*100:+.2f} pp   (Run B4={RUNB4_R2})')
print('=' * 72)
if gate_r2_ok and gate_spear_ok:
    print('  PASS H1.C  →  H1 est défendable.')
    print('                 Argument : mix=0.05 préserve les performances tout en')
    print('                 fournissant une carte M algébriquement exacte.')
    print('                 Prochaine étape : analyser cartes M (H1.A, H1.D).')
else:
    failed = []
    ...
    print('                 5 runs successifs FAIL avec toutes les variantes.')
    print('                 Conclusion : l\\'architecture additive est incompatible.')
    print('                 → Pivot H2 (TsSHAP / SHAPformer).')
print('=' * 72)"""
)
...
plt.suptitle(
    f'Evidence Maps M — Run B5 (mix={EVIDENCE_MIX_TARGET}, tous les fixes)\\n'
...
## Configuration — tous les fixes combinés
...
| **B5** | **→0.05** | **✅** | **✅** | **✅** | **{test_r2:.4f}** | **{test_spear:.4f}** |
